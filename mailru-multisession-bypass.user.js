...
(function() {
    ...
    function mutateConfig(cfg) {
        if (!cfg) return;
        try {
            if (cfg.cfg && cfg.cfg.omicron && cfg.cfg.omicron.Config) {
                cfg.cfg.omicron.Config['multi-cookie-restriction-enabled'] = false;
                cfg.cfg.omicron.Config['show-party-informer'] = false; 
            }
        } catch(e) {}
    }

    try {
        const descriptor = Object.getOwnPropertyDescriptor(window, 'sotaConfig');
        ...
        if (!descriptor || descriptor.configurable) {
            ...
            console.warn('[Mail.ru Bypass] sotaConfig защищен от перезаписи. Использую прямую мутацию.');
            mutateConfig(window.sotaConfig);
        }
    } catch (e) {
        console.warn('[Mail.ru Bypass] Ошибка при установке ловушки на sotaConfig:', e);
        mutateConfig(window.sotaConfig);
    }
